...
const getAllBugs = async (target, callback) => {
  try {
    let listOfIssues = []
    jira.search.search({
        jql: `project in (Dynamo,"Growth Team","Infrastructure & Support","Web Dev","Feature Team") and "Target[Select List (multiple choices)]" IN ("${target}") and type = Bug and creator = "Deep Clone for Jira" ORDER BY created DESC`,
        maxResults: 1000
    }, (error, result)  => {
